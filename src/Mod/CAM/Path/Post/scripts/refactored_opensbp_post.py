...
class Refactored_Opensbp(PostProcessor):
    ...
    * By default, this post-processor fails if it sees a gcode it doesn't understand. but see --abort-on-unknown and --skip-unknown
    ...
    * This always sets the move & jog speeds (feed and rapid) at the beginning of an operation, by fetching the Tool speeds (because some operations omit initial speeds). If rapid-speeds are 0.0, it will not set the jog-speed, so will use the machine setting.
    ...
    * For spindle speed changes, your ShopBot C6 "cut" command should be configured for having-a-speed-control-module, or for not having. It will prompt you if you don't have the module.
    ...
    def set_speed( self, path_command ):
        ...
        if native_command == 'MS':
            ...
            if self.post.arguments.speed_modal:
                print(f"### --speed-modal was {self.current_location['ms']} vs new {speeds}")
                for i,new_speed in enumerate( speeds ):
                    old_speed = self.current_location[native_command.lower()][i]
                    print(f"###   old,new {old_speed.__class__.__name__}:{old_speed},{new_speed.__class__.__name__}:{new_speed} ==? {old_speed == new_speed}")
